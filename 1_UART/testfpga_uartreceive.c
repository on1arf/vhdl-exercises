...
// F_UART_READDATA: FPGA "readdata" pin (PC13): OUTPUT
#define GPIO_F_UART_READDATA GPIOC
#define GPIO_F_UART_READDATAp GPIO13

// F_UART_TXDA: FPGA "Data Available" pin (PC14): INPUT
#define GPIO_F_UART_TXDA GPIOC
#define GPIO_F_UART_TXDAp GPIO14
...
gpio_set_mode(GPIO_F_UART_READDATA, GPIO_MODE_OUTPUT_2_MHZ, GPIO_CNF_OUTPUT_PUSHPULL, GPIO_F_UART_READDATAp);

// Set pin PC14  for input "UART buffer_empty"
// enable pull down
gpio_set_mode(GPIO_F_UART_TXDA, GPIO_MODE_INPUT, GPIO_CNF_INPUT_PULL_UPDOWN, GPIO_F_UART_TXDAp);
gpio_clear(GPIO_F_UART_TXDA,  GPIO_F_UART_TXDAp);
...
// mark anything still on the bus as read
gpio_set(GPIO_F_UART_READDATA,GPIO_F_UART_READDATAp);
wait_ms(1);					
gpio_clear(GPIO_F_UART_READDATA,GPIO_F_UART_READDATAp);
wait_ms(1);					
...
while(FOREVER) {
	...
	for (c=0; c<=0xff; c++) {
		for (c2=0; c2<REPEAT; c2++) {
			// Make data
			snprintf(printtxt,80,"setting %02X on the serial port! ",c);
			write_uart1(printtxt,strlen(printtxt));

			snprintf(printtxt,80,"bin: "BYTE_TO_BINARY_PATTERN" ", BYTE_TO_BINARY(c));
			write_uart1(printtxt,strlen(printtxt));
			...
			// wait up to 1 second to receive data from port
			sendStartTime = system_millis;


			success=0;
			while(1) {
				success = gpio_get(GPIO_F_UART_TXDA,GPIO_F_UART_TXDAp);

				if (success) {
					// Got data
					ret=(gpio_port_read(GPIO_F_UART_BUS) & 0xFF);
			...
			// toggle "uart_readdata" pin high and low to indicated data has been read
			gpio_set(GPIO_F_UART_READDATA,GPIO_F_UART_READDATAp);
			wait_ms(1);					
			gpio_clear(GPIO_F_UART_READDATA,GPIO_F_UART_READDATAp);
			wait_ms(1);					
			...
			if (success) {
				if (ret == (c & 0xFF)) {
					...
					snprintf(printtxt,80,"Received: NOK : "BYTE_TO_BINARY_PATTERN"\n", BYTE_TO_BINARY(ret));
				}; // end else - if

				write_uart1(printtxt,strlen(printtxt));
				wait_ms(20);					
			} else {
				snprintf(printtxt,80,"No data received from uart bus (no TX Data Available indication)\n");
				write_uart1(printtxt,strlen(printtxt));
			}; // end else - if
